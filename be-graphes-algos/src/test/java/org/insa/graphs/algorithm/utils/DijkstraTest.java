...
public class DijkstraTest {
    ...
    ModeParcours mode = ModeParcours.AStar;
    @Test
    ...
    @Test
    // Vérifie que le chemin construit par l'algo est valide en comparant avec Bellman.
    public void testDijkstraCheminValidePietonEtCycliste() throws Exception {
        final String mapInsa = "C:\\Users\\keke3\\Downloads\\insa.mapgr";
        final GraphReader readerInsa = new BinaryGraphReader(
                new DataInputStream(new BufferedInputStream(new FileInputStream(mapInsa))));
        final Graph graphInsa = readerInsa.read();
        final Node nodeOrigin = graphInsa.get(700);
        final Node nodeDestination = graphInsa.get(253);
        final ShortestPathData data = new ShortestPathData(graphInsa, nodeOrigin, nodeDestination, ArcInspectorFactory.getAllFilters().get(4));
        final DijkstraAlgorithm algo = new DijkstraAlgorithm(data);
        final ShortestPathSolution solutionAlgo = algo.run();
        final BellmanFordAlgorithm bellman = new BellmanFordAlgorithm(data);
        final ShortestPathSolution solutionBellman = bellman.run();
        assertEquals(solutionBellman.getPath().getLength(), solutionAlgo.getPath().getLength(), 0.0001);
    }
    ...
    public void testCheminValideNonValidePourVoiture() throws IOException {
        final String map = "C:\\Users\\keke3\\Downloads\\insa.mapgr";
        final GraphReader reader = new BinaryGraphReader(
                new DataInputStream(new BufferedInputStream(new FileInputStream(map))));
        final Graph graphInsa = reader.read();
    ...
    public void testComparaisonVitesseBellman() throws IOException {
        final String map = "C:\\Users\\keke3\\Downloads\\toulouse.mapgr";
        final GraphReader reader = new BinaryGraphReader(
                new DataInputStream(new BufferedInputStream(new FileInputStream(map))));
        final Graph graphInsa = reader.read();
        final Node nodeOrigin = graphInsa.get(100);
